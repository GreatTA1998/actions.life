...
export function createCalendarService ({ treesByDate, treesByID }) {
  const listeners = {}

  function setupCalListener (leftDT, rightDT) {  
  ...
  return { setupCalListener }
}

// Pure helpers — no dependency on instance state
function divideIntoRegions (leftISO, rightISO, chunkSize) {
  ...
  while (currentDate <= endDate) {
    currentChunk.push(currentDate.toFormat('yyyy-MM-dd'))
