<script>
  ...
  function updateLayout(newVal) {
    Task.update({ id: taskObject.id, keyValueChanges: { photoLayout: newVal }})
  }
</script> 

<div>
  <ToggleGroupStyled>
    <button onclick={() => updateLayout('side-by-side')} class="toggle-btn" class:active={taskObject.photoLayout === 'side-by-side'}>
      <MslSplitscreenLeft style="font-size: 1.25rem;"/>
    </button>
    <button onclick={() => updateLayout('top-and-below')} class="toggle-btn" class:active={taskObject.photoLayout === 'top-and-below'}>
      <MslSplitscreenTop style="font-size: 1.25rem;"/>
    </button>
    <button onclick={() => updateLayout('full-photo')} class="toggle-btn" class:active={taskObject.photoLayout === 'full-photo'}>
