...
- The landing page "Playground" uses anonymous Firebase auth and renders the full `UserAppInstance`, so it can be used for manual testing without a real Google login. It's essentially the full app except for minor edge cases from being embedded rather than fullscreen.
